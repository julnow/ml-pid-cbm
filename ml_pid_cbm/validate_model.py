...
class ValidateModel:
    ...
    print_output: bool = True,
        ...
        Purity is calculated as correctly identified X / all identified X

        Args:
            cnf_matrix (np.ndarray): Confusion matrix generated by sklearn.metrics.confusion_matrix.
            pid (int): Pid of particles to print efficiency stats.
            txt_tile (io.TextIOWrapper): Text file to write the output. Defaults to None.
            print_output (bool): Whether to print the output to stdout. Defaults to True.

        Returns:
            Tuple[float, float]: Tuple with efficiency and purity
        """
        all_simulated_signal = cnf_matrix[pid].sum()
        true_signal = cnf_matrix[pid][pid]
        false_signal = cnf_matrix[:, pid].sum() - true_signal
        reconstructed_signals = true_signal + false_signal

        efficiency = (true_signal / all_simulated_signal) * 100
        purity = (true_signal / reconstructed_signals) * 100
